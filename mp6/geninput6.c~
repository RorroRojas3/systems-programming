...
{ 
    ...
    for(i = 0; i < num_records; i++) 
    {
        ...
        // Creates different value from 0 to 1000 and stores it to rec_ptr.value
        rec_ptr[i].value = 1000 * drand48();
        
        // Creates different values from 0 to 10 and stores it to rec_ptr.a
        for (j = 0; j < NUMINTS; j++)
        {
            rec_ptr[i].a[j] = 10 * drand48(); 
        }
        
        // Creates a number between 2 and 7 and stores it into c1
        while (c1 < 2 || c1 > 7)
        {
        	c1 = drand48() * 7;
        }
        
        // Creates a number between 65 and 90 and stores it in x and that value is stored into rec_ptr.name
        for (c2 = 0; c2 < c1; c2++)
        {
        	x = (drand48() * 25) + 65;
       	     	rec_ptr[i].name[c2] = x;
